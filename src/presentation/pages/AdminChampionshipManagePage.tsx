...
type RefereeOption = { id: string; name: string; nickname: string | null; };
...
export function AdminChampionshipManagePage() {
  ...
  // ── Rodadas tab state ────────────────────────────────────────────────────
  type RoundMatchRow = { home_team_id: string; away_team_id: string; match_date: string; venue_id: string; referee_id: string; assistant_1_id: string; assistant_2_id: string; };
  ...
  const [roundRows, setRoundRows] = useState<RoundMatchRow[]>([{ home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }]);
    ...
    try {
      const payload = {
        matches: validRows.map(r => ({
          ...
          main_referee_id: r.referee_id || null,
          assistant_1_id: r.assistant_1_id || null,
          assistant_2_id: r.assistant_2_id || null,
        })),
      };
      ...
      if (result.errors === 0) {
        setRoundRows([{ home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }]);
      }
    } catch (err) {
      setSubmitError((err as Error).message);
    } finally {
  ...
  function switchToTab(tab: "estrutura" | "inscritos" | "rodadas") {
    ...
    if (tab === "rodadas" && !refereesLoaded) {
      fetch("/api/referees", { headers: authHeaders() })
        .then(r => r.json())
        .then((data: RefereeOption[]) => { setReferees(data); setRefereesLoaded(true); })
        .catch(() => setRefereesLoaded(true));
    }
  }
          ...
          <section style={S.section}>
            ...
            <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "1rem" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                ...
                <select
                  value={roundPhaseId}
                  onChange={e => { setRoundPhaseId(e.target.value); setRoundGroupId(""); setRoundRows([{ home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }]); setRoundResult(null); }}
                  style={S.input}
                >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                ...
                <select
                  value={roundGroupId}
                  onChange={e => { setRoundGroupId(e.target.value); setRoundRows([{ home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }]); setRoundResult(null); }}
                  style={S.input}
              ...
              return (
                <div style={{ marginBottom: "1rem" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                    ...
                    <button
                      style={S.btnAddSmall}
                      onClick={() => setRoundRows(prev => [...prev, { home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }])}
                    >
                      + Partida
                    </button>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {roundRows.map((row, idx) => (
                      <div key={idx} style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap", backgroundColor: "#181825", border: "1px solid #313244", borderRadius: "6px", padding: "0.6rem 0.75rem" }}>
                        ...
                        {/* Referee row */}
                        <div style={{ width: "100%", display: "flex", gap: "0.5rem", flexWrap: "wrap", paddingTop: "0.4rem", borderTop: "1px solid #313244", marginTop: "0.25rem" }}>
                          <select
                            value={row.referee_id}
                            onChange={e => setRoundRows(prev => prev.map((r, i) => i === idx ? { ...r, referee_id: e.target.value } : r))}
                            style={{ ...S.inputSmall, flex: 1, minWidth: "140px" }}
                          >
                            <option value="">⚖️ Árbitro principal</option>
                            {referees.map(r => <option key={r.id} value={r.id}>{r.name}{r.nickname ? ` (${r.nickname})` : ""}</option>)}
                          </select>
                          <select
                            value={row.assistant_1_id}
                            onChange={e => setRoundRows(prev => prev.map((r, i) => i === idx ? { ...r, assistant_1_id: e.target.value } : r))}
                            style={{ ...S.inputSmall, flex: 1, minWidth: "140px" }}
                          >
                            <option value="">🚩 Assistente 1</option>
                            {referees.map(r => <option key={r.id} value={r.id}>{r.name}{r.nickname ? ` (${r.nickname})` : ""}</option>)}
                          </select>
                          <select
                            value={row.assistant_2_id}
                            onChange={e => setRoundRows(prev => prev.map((r, i) => i === idx ? { ...r, assistant_2_id: e.target.value } : r))}
                            style={{ ...S.inputSmall, flex: 1, minWidth: "140px" }}
                          >
                            <option value="">🚩 Assistente 2</option>
                            {referees.map(r => <option key={r.id} value={r.id}>{r.name}{r.nickname ? ` (${r.nickname})` : ""}</option>)}
                          </select>
                        </div>
                      </div>
                  ...
                  <div style={{ marginTop: "1rem", display: "flex", gap: "0.75rem" }}>
                    ...
                    <button style={S.btnCancel} onClick={() => { setRoundRows([{ home_team_id: "", away_team_id: "", match_date: "", venue_id: "", referee_id: "", assistant_1_id: "", assistant_2_id: "" }]); setRoundResult(null); setSubmitError(null); }}>
                      Limpar
                    </button>
                  </div>
                </div>
